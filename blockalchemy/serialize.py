...
def ser_vector(l):
    ...
    for i in l:
        ...
        if type(i) is TxIn:
            r += ser_txin(i)
        elif type(i) is TxOut:
            r += ser_txout(i)
        elif type(i) is Transaction:
            r += ser_tx(i)
        else:
            raise NotImplementedError
...
def ser_block(blk):
    ...
    r += ser_vector(blk.transaction_list, Transaction)
...
def deser_history_row(f):
    from models import history_row
...
def deser_address_update(f):
    ...
    from models import address_update
